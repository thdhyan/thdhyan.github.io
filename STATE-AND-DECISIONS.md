...
- **Fleet = raw URDFs at runtime** (`src/urdf.js` + vite middleware `serveModelsRaw`): real colors/textures, joints poseable, floor re-align after every pose change (idempotent `realign()`), dev hooks `window.__fleet` + `window.__THREE`. Hero + drone stay GLB. Single hero canvas only — card images are static renders (no per-card canvases).
...
- **QA tools (all green 2026-09-22)**: `tools/posecheck.mjs`, `tools/editorcheck.mjs`, `tools/identity.mjs`, `tools/cardscheck.mjs`, `tools/rendercards.mjs`, `tools/urdf-diag.mjs` + `npx playwright test tests/portfolio.spec.ts --project=chromium` + `npm run lint`.
- **STILL TODO**: hero→cards scroll choreography (decision 4, 2nd pass); optional: renders for the 5 cards without a robot.
- `docs/` rebuilt from this work 2026-09-22 via `npm run deploy`.
